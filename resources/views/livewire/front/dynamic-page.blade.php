<div>
    @section('title', $page->title . ' — Asosiasi Arsiparis Indonesia')
    @section('meta_description', $page->seo_description ?: Str::limit(strip_tags($page->content), 160))
    ...
    <section class="py-20 bg-white dark:bg-slate-900 min-h-[40vh]">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 prose dark:prose-invert prose-amber lg:prose-lg text-slate-700 dark:text-slate-200 leading-relaxed">
            @if(empty(strip_tags($page->content)))
                <div class="glass-card p-12 rounded-3xl text-center border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
                    ...
                    <p class="text-slate-600 dark:text-slate-400">Konten untuk halaman <strong>{{ $page->title }}</strong> sedang disusun oleh tim Sekretariat Nasional AAI. Silakan kunjungi kembali nanti.</p>
                </div>
            @else
                {!! $page->content !!}
            @endif
